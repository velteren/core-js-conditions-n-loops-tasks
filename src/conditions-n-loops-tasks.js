...
function getSpiralMatrix(size) {
  ...
  while (iterator < size * size) {
    ...
    while (colIndex < size && result[rowIndex][colIndex] === undefined) {
      iterator += 1;
      result[rowIndex][colIndex] = iterator;
      colIndex += 1;
    }
    ...
    while (rowIndex < size && result[rowIndex][colIndex] === undefined) {
      iterator += 1;
      result[rowIndex][colIndex] = iterator;
      rowIndex += 1;
    }
    ...
    while (colIndex >= 0 && result[rowIndex][colIndex] === undefined) {
      iterator += 1;
      result[rowIndex][colIndex] = iterator;
      colIndex -= 1;
    }
    ...
    while (rowIndex >= 0 && result[rowIndex][colIndex] === undefined) {
      iterator += 1;
      result[rowIndex][colIndex] = iterator;
      rowIndex -= 1;
    }
...
function getNearestBigger(number) {
  function getStringToCompare(n) {
    const arr = [...`${n}`];
    arr.sort();
    const sorted = arr.reduce((acc, item) => acc + item, '');
    arr.reverse();
    const biggest = arr.reduce((acc, item) => acc + item, '');
    return [sorted, biggest];
  }

  const [sortedStr, biggest] = getStringToCompare(number);
  if (+biggest === number) return number;
  let tmp = number;
  for (;;) {
    tmp += 1;
    if (sortedStr === getStringToCompare(tmp)[0]) {
      return tmp;
    }
  }
}
